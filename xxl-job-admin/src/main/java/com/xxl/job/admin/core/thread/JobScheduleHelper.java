...
public class JobScheduleHelper {
  ...
  private Thread scheduleStreamingThread;
  private volatile boolean scheduleThreadToStop = false;
  private volatile boolean ringThreadToStop = false;
  private volatile boolean scheduleStreamingThreadToStop = false;
  private static volatile Map<Integer, List<Integer>> ringData = new ConcurrentHashMap<>();

  public void start() {

    scheduleStreamingThread =
        new Thread(
            () -> {
              try {
              ...
              int preReadCount =
                  (XxlJobAdminConfig.getAdminConfig().getTriggerPoolFastMax()
                      ...
                      * 20;
              while (!scheduleStreamingThreadToStop) {
                ...
                try {
                  ...
                  preparedStatement =
                      conn.prepareStatement(
                          "select * from xxl_job_lock where lock_name = 'schedule_streaming_lock' for update");
                  ...
                  List<XxlJobInfo> scheduleList =
                      XxlJobAdminConfig.getAdminConfig()
                          .getXxlJobInfoDao()
                          .scheduleStreamingJob(nowTime + PRE_READ_MS, preReadCount);
                  if (!CollectionUtils.isEmpty(scheduleList)) {
                    for (XxlJobInfo xxlJobInfo : scheduleList) {
                      JobTriggerPoolHelper.trigger(
                          xxlJobInfo.getId(), TriggerTypeEnum.CRON, -1, null, null, null);
                    }
                  }
                  ...
                  if (!scheduleStreamingThreadToStop) {
                    logger.error(
                        ">>>>>>>>>>> xxl-job, JobScheduleHelper#scheduleStreamingThread error:{}",
                        e);
                  }
                } finally {
                  // commit
    ...
    scheduleStreamingThread.setDaemon(true);
    scheduleStreamingThread.start();
    scheduleStreamingThread.setName("xxl-job, admin JobScheduleHelper#scheduleStreamingThread");
  ...
  public void toStop() {
    ...
    scheduleStreamingThreadToStop = true;
    try {
      TimeUnit.SECONDS.sleep(1); // wait
    } catch (InterruptedException e) {
      logger.error(e.getMessage(), e);
    }
    if (scheduleStreamingThread.getState() != Thread.State.TERMINATED) {
      // interrupt and wait
      scheduleStreamingThread.interrupt();
      try {
        scheduleStreamingThread.join();
      } catch (InterruptedException e) {
        logger.error(e.getMessage(), e);
      }
    }
